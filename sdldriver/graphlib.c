...
#define PLOT(X,Y,C) screen[(X)+(Y)*SCREEN_W]=C
#define GETPLOT(X,Y) screen[(X)+(Y)*SCREEN_W]
#define PI M_PI
...
int set_graph_mode(int w, int h)
{
    SCREEN_W = w;
    SCREEN_H = h;
...
int modo13h()
{
	return set_graph_mode(320,200);
}

int init_sdldriver(int w, int h)
{
	return set_graph_mode(w,h);
}
...
void fillscreen(char a)
{
	memset(screen,a,SCREEN_W*SCREEN_H);
}
