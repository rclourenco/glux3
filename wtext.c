...
void wtext_draw(TObjectUI *o, int x, int y)
{
    ...
    char otb[2] = {0,0};
    gx->setfillstyle(SOLID_FILL,0);
    gx->setcolor(2);
    gx->rectangle(x1,y1,x2,y2);

    gx->bar(x1+1,y1+1,x2-1,y2-1);

    if(o->flags&WFOCUS) {
	gx->setcolor(4);
	gx->rectangle(x1+1,y1+1,x2-1,y2-1);
	gx->setcolor(2);
    }
	...
	if( t->buffer[i]=='\n' ) {
		...
		ty = y1+(l-t->sl)*gx->textheight("²")+5;
		tx = x1+(c-t->sc)*gx->textwidth("Í")+5;

		if( c >= t->sc && c< t->nc+t->sc && l >= t->sl &&  l < t->nl+t->sl )
		{
			otb[0]=t->buffer[i];
			gx->outtextxy(tx,ty,otb);
		}
		c++;
	}
